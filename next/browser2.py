...
class Browser:
    ...
    def _get_page_state(self):
                        ...
                        if element.is_visible():
                            ...
                            if is_in_viewport(bounding_box, viewport_width, viewport_height):
                                ...
                                interactive_elements.append({
                                    ...
                                    'type': element_type,
                                    'text': text,
                                    'visible': True,  # Confirmed visible and in viewport
                                    'href': href,
        ...
        for link in link_elements:
            try:
                # Check if link is generally visible
                if link.is_visible():
                    # Get the bounding box relative to the viewport
    ...
    def crawl(self):
        ...
        for i, link in enumerate(page_state['links'], 1):
            print(f"{i:2d}. {link['text'][:40]}{'...' if len(link['text']) > 40 else ''} -> {link['href']}")
    ...
    def fill_input(self, element_id, text):
        ...
        element.fill(text)
        _browser.enter()
        print(f"Filled element '{element_id}' with text: {text}")
        return True
...
if __name__ == "__main__":
    ...
    try:
        ...
        # You can uncomment the interactive loop below to test further
        # while True:
        #     option = input("""
        # Options:
